admin.controller("nhaphangController", ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.CreatePrint = function () {
        ...
        //Thực đơn
        angular.forEach($scope.chitietNhapHangs, function (value, index) {
            angular.forEach($scope.mathangs, function (mathang, indexMatHang) {
                if (value.idMatHang == mathang.id) {
                    var chitietNhapHang = angular.copy(value);
                    chitietNhapHang.tenMatHang = mathang.ten;
                    $scope.print.chitietNhapHangs.push(chitietNhapHang);
                }
            });
        });
    }
    ...
    $scope.getDate = function () {
        ...
        $scope.dayNow = dateNow.getDay();
        $scope.monthNow = dateNow.getMonth();
        $scope.yearNow = yearNow.getFullYear();
    }
}]);